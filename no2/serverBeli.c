...
void *beli(void *arg){
    ...
    while(/*read(*client_socket,client_response,strlen(client_response))*/recv(*client_socket,&client_response,sizeof(client_response),0)){ 
        ...
        if(strcmp(client_response,"beli") == 0){
            
            if(*stock > 0){
                *stock=*stock-1;
                char message[100] = "transaksi berhasil";
                ...
                char message[100] = "transaksi gagal";
                if(send(*client_socket,message,sizeof(message),0)<0){
                    printf("gagal\n");
                }
...
int main(){
    ...
    *stock = 2;
    ...
    int err=pthread_create(&thread,NULL,beli,(void *)&client_socket);
    if(err){
        printf("failed\n");
    }
